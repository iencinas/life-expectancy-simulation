...
#missing!! and more --ivan
all_causes%>%filter(Age==85)
...
mx_age <- mx_age %>%
  group_by(Age,ICD_chapter) %>%
  summarise(Deaths = sum(Deaths), Population = max(Population)) %>%
  mutate(Crude_rate = (Deaths / Population) * 100000)

#rando checks to know whats going on --ivan
mx_age%>%group_by(ICD_chapter)%>%summarise(n())
tail(data.frame(mx_age),n=100)
mx_age%>%filter(Age==85)
...
saveRDS(mx_age,file='data/mx_age.rds')
